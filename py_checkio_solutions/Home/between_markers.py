...
def between_markers(text: str, begin: str, end: str) -> str:
    ...
    loc_begin = text.find(begin)
    loc_end = text.find(end)

   # if openning tag not found, we start with 0 instead of -1
    if not loc_begin == -1:
        # to accommodate the length of the tag
        loc_begin += len(begin)
    else:
        loc_begin = 0

    # if closing tag not found, we will end of the length of the text
    if loc_end == -1:
        loc_end = len(text)

    if loc_end > loc_begin:
        a = text[loc_begin:loc_end]
        # print(a)
        # print("b: " + str(loc_begin) + " e: " + str(loc_end))
        return a
    else:
        return ""
