...
Interpreter.prototype.resolveLiterial = function(varName) {
  const resolvedVars = this.literals[varName];
  if (resolvedVars === undefined) {
    throw new LiteralError(`ERROR: Invalid identifier. No variable with name '${varName}' was found.`)
  }
  return { type: 'numbers', value: resolvedVars }
}
...
Interpreter.prototype.tokenize = function (program) {
  ...
  tokens = tokens.map((t) => {
    if (isOperators(t)) {
      return { type: 'operators', value: t };
    } else if (isNumbers(t)) {
      const num = parseFloat(t);
      if (!isFinite(num)) throw new Error('number is too large');
      return { type: 'numbers', value: num };
    } else if (isLiterals(t)) {
      return { type: 'literals', value: t };
    } else {
...
Interpreter.prototype.parse = function (tokens) {
  ...
  // resolve known literals
  tokens = tokens.map(v => (v.type === 'literals') ? this.resolveLiterial(v.value) : v);
  ...
  postfix.forEach(t => {
    if (t.type === 'numbers') {
      ...
      if (t.value === '--') {
        ...
        obj.value = this.ops[t.value](op1, op2);
      }
      stack.push(obj);
    }
  });
...
Interpreter.prototype.input = function (expr) {
  ...
  try {
    const assignmentId = tokens.findIndex(v => v.type === 'operators' && v.value === '=');
    let assignmentTokens = [];
    if (~assignmentId) {
      assignmentTokens = tokens.splice(0, assignmentId + 1);
    }
    ...
    if (~assignmentId) {
      const varName = assignmentTokens[assignmentId - 1].value;
      this.literals[varName] = result;
    }
    return result;
  } catch (e) {
    throw (e);
...
// log(interpreter.input("8 / 4"), 2);
// log(interpreter.input("7 % 4"), 3);
// log(interpreter.input("7"), 7);

// //Variables
// log(interpreter.input("x = 1"), 1);
// log(interpreter.input("x"), 1);
// log(interpreter.input("x + 3"), 4);
log(interpreter.input(""));  //error
log(interpreter.input("y"));  //error

log(interpreter.input("x = 7"), 7);
log(interpreter.input("x"), 7);  //error
log(interpreter.input("y = x + 5"), 12);
// log(interpreter.input("y", 12));  //error
log(interpreter.input("x + y"), 19);
...
// Interpreter.prototype.parseLiterals = function (tokens) {

//   const tokenId = tokens.findIndex(v => v.type === 'literals');
//   const varName = tokens[tokenId].value;
//   const v = tokens[tokenId + 1];  // check for assignment
//   if (v !== undefined && v.type === 'operators' && v.value === '=') {
//     // assignment
//     this.literals[varName] = tokens[tokenId + 2].value;
//     tokens.splice(tokenId, 2);  //remove varName and assignemnt tokens
//   } else {
//     // var read
//     const resolvedVars = this.literals[varName];
//     if (resolvedVars === undefined) {
//       throw new Error(`ERROR: Invalid identifier. No variable with name '${varName}' was found.`)
//     } else {
//       tokens[tokenId] = { type: 'numbers', value: resolvedVars }
//     }
//   }
//   return tokens;
// }